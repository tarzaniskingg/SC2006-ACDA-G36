    ...
    return (
      <div className="space-y-0.5">
        <div className="relative h-3 flex rounded-full overflow-hidden">
          {blocks.map((b, i) => (
            <div key={i} className="flex-1" style={{ backgroundColor: crowdColor(b.level) }}
              title={`${b.hour}:${String(b.min).padStart(2, '0')} — ${b.level}`} />
          ))}
          {nowH >= 6 && nowH <= 23 && (
            <div className="absolute top-0 bottom-0 w-0.5 bg-white" style={{ left: `${nowPct}%` }} title="Now" />
          )}
        </div>
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-2 text-[10px] text-slate-400">
        <span className="font-medium font-display">{label || stationName}</span>
      </div>
      <div className="relative h-5 flex rounded-lg overflow-hidden">
        {blocks.map((b, i) => (
          <div key={i} className="flex-1 transition-colors" style={{ backgroundColor: crowdColor(b.level) }}
            title={`${b.hour}:${String(b.min).padStart(2, '0')} — ${b.level}`} />
        ))}
        {nowH >= 6 && nowH <= 23 && (
          <div className="absolute top-0 bottom-0 w-0.5 bg-white/80" style={{ left: `${nowPct}%` }}>
            <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 text-[8px] font-bold text-white/70 font-display">
              Now
            </div>
          </div>
        )}
      </div>
      <div className="flex justify-between text-[9px] text-slate-500 font-display">
        <span>6 AM</span><span>9 AM</span><span>12 PM</span><span>3 PM</span><span>6 PM</span><span>9 PM</span><span>11 PM</span>
      </div>
    </div>
  );
}
