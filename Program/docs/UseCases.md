...
| Priority | High |
| Frequency of Use | High |
| Flow of Events | 1. System identifies which data sources are needed based on the enabled route categories and route segments. 2. For each required source, system checks the in-memory TTL cache with per-key locking (only one thread fetches on cache miss; others wait for the result). If the cached entry is fresh (within its TTL threshold), it is used directly. 3. If the cache entry is expired or missing, system fetches fresh data from the external API: (a) LTA Bus Arrival API — bus crowding and arrival times (TTL: 30s); (b) LTA PCD Real Time API — real-time MRT station crowding levels (TTL: 60s); (c) LTA PCD Forecast API — forecasted MRT station crowding by 30-min interval (TTL: 6h, updated daily); (d) LTA Train Service Alerts — MRT delay/disruption status (TTL: 60s); (e) LTA Carpark Availability — parking lot counts near destination (TTL: 300s); (f) NEA 2-Hour Weather Forecast — rain prediction along route (TTL: 600s); (g) Google Directions API — route options with duration_in_traffic (traffic-aware driving ETA) and fare data (real TransitLink pricing for transit). 4. A global rate limiter ensures LTA API calls do not exceed ~6 requests/second to stay within quota limits. 5. Each response is stored in the cache with its retrieval timestamp. 6. If an API call fails, system falls back to the most recent cached data (even if expired) and marks it with `is_fallback = true` and `source = "fallback"`. If no cached data exists at all, an empty structure with `is_fallback = true` is used. 7. Different segments within the same route may independently use real-time or fallback data. If any segment uses fallback data, the route-level `uses_fallback` flag is set to true. 8. The Settings screen displays each data source status as "Live" (green) or "Fallback" (amber). |
| Alternative Flows | NIL |
...
| Priority | High |
| Frequency of Use | High |
| Flow of Events | 1. For each bus segment, system computes a crowding indicator from the LTA Bus Arrival API Load field: SEA (Seats Available) = Low, SDA (Standing Available) = Medium, LSD (Limited Standing) = High. The same Bus Arrival response is reused for both crowding and frequency assessment (no duplicate API call). 2. For each MRT segment, system attempts three sources in order: (a) LTA PCD Real Time API — returns current crowding level per station (l=Low, m=Medium, h=High); (b) LTA PCD Forecast API — returns forecasted crowding by 30-min interval matched to the query time; (c) Time-based heuristic — uses well-known Singapore MRT peak patterns (weekday 7-9:30am and 5:30-8pm = High, shoulder periods = Medium, off-peak = Low, weekends midday = Medium). The first source that returns valid data is used. 3. For driving/taxi segments, crowding is set to Unknown (not applicable). 4. System aggregates segment-level values to a route-level crowding risk using a worst-case (maximum) rule: (a) if all segments are Unknown, the route is Unknown with numeric value 2; (b) if some are Unknown and some known, the route-level value is the maximum of the known values; (c) otherwise, the highest segment risk is used. 5. Route-level crowding risk is passed to UC-11 for scoring. |
| Alternative Flows | NIL |
| Exceptions | NIL |
| Includes | NIL |
...
| Actor | System |
| Description | Evaluates the likelihood of delays on each route segment. For MRT segments, uses train service alerts. For driving/taxi segments, derives delay risk from Google's traffic-aware duration. Bus segments default to Low. |
| Preconditions | Transport data has been collected (UC-07) and routes have been generated (UC-10) |
| Postconditions | Segment-level and route-level delay risk indicators are assigned |
| Priority | High |
| Frequency of Use | High |
| Flow of Events | 1. For each MRT segment, system computes a delay indicator from the LTA Train Service Alerts API: Status=1 (normal) = Low; Status=2 (disruption) on the route's line = High; Status=2 on a different line = Low; Status=2 with no specific line identified = Medium (precautionary). 2. For each bus segment, system assigns a default delay indicator of Low. 3. For each driving/taxi segment, system derives delay risk from Google's traffic data by comparing the baseline duration to the traffic-aware duration (duration_in_traffic): ratio >= 1.5 (50%+ slower) = High, ratio >= 1.2 (20-50% slower) = Medium, ratio < 1.2 = Low. This requires no additional API calls since Google Directions already provides both values. 4. If delay data is unavailable for a segment, it is marked Unknown. 5. System aggregates to route-level delay risk using the same worst-case rule as crowding (UC-08 step 4). 6. Route-level delay risk is passed to UC-11 for scoring. |
| Alternative Flows | If Google does not provide duration_in_traffic for a driving segment, delay is marked Unknown. |
| Exceptions | NIL |
| Includes | NIL |
...
| Priority | High |
| Frequency of Use | High |
| Flow of Events | 1. System reads the active preference configuration to determine which modes are enabled. 2. If Public Transit is enabled, system calls the Google Directions API with mode "transit" to generate public transit route options (may include MRT-only or MRT+Bus combinations). Google transit and driving calls are made in parallel for speed. 3. If Taxi/Drive is enabled, system calls the Google Directions API with mode "driving". For each driving route, system generates two candidates: (a) a Taxi candidate (no parking penalty, CDG 2026 metered fare with surcharges) and (b) a Drive candidate (fuel cost, with parking penalties on time/comfort/risk from nearby carpark availability). 4. All route candidates are assessed in parallel using a thread pool. 5. System deduplicates candidate routes by fingerprint (category + mode sequence + line names + stops), retaining the faster route when duplicates exist. 6. System filters out routes that violate the maximum walking allowance, maximum transfers, or maximum budget constraints. 7. For each feasible route, system assigns: estimated travel time (using Google's duration_in_traffic for driving routes when available, providing real-time traffic-aware ETAs), realistic travel time (adding 0.5 * bus miss_penalty_min per bus step and parking search penalties), number of transfers (transit segments minus one), estimated cost (using Google's fare field for transit when available — includes real TransitLink pricing with early-bird and off-peak discounts — falling back to distance-based fare table with time-of-day adjustments), walking time, weather data (rain warnings from NEA), ERP charges (from polyline-gantry proximity matching), parking availability (from LTA Carpark API), and route-level risk indicators from UC-08 and UC-09. 8. Candidate routes are passed to UC-11 for scoring and UC-12 for ranking. |
...
| Actor | User |
| Description | Allows the user to compare how a selected route category (Public Transit, Taxi, or Drive) performs across different departure time windows (Morning Rush, Around Now, Evening Rush) for an apples-to-apples comparison. |
| Preconditions | Routes have been generated and displayed (UC-14), and the user has selected a route (UC-15) |
| Postconditions | The user can see route scores, fares, travel times, and crowding across 9 departure time slots for the selected route category |
| Priority | Medium |
| Frequency of Use | Low to Medium |
| Flow of Events | 1. When the user selects a route, the system begins pre-fetching compare data in the background for that route's category. Compare data is cached per category so switching between routes of the same type (e.g. two Public Transit routes) does not trigger a new fetch. 2. A floating compare button appears at the bottom of the screen showing the selected category (e.g. "Compare Public Transit Departure Times"). 3. User taps the floating compare button. 4. If pre-fetched data is available, the modal opens instantly. Otherwise, the system calls the /routes/compare endpoint with origin, destination, category, and weight preferences. 5. Backend generates routes for 9 time slots across 3 groups: Morning Rush (07:30, 08:00, 08:30), Around Now (current time, +30 min, +60 min), and Evening Rush (17:30, 18:00, 18:30). All 9 slots are fetched in parallel. 6. For each time slot, backend filters to only routes matching the selected category, then ranks them and returns the top 3 with scores, times, costs, and crowding levels. Transit fares use Google's real fare data when available, and crowding uses LTA real-time data or time-based heuristics. 7. Frontend displays the results as a grouped comparison table with colour-coded cells (green = best, red = worst) and a category badge in the header. 8. User reviews the comparison and closes the modal. |
| Alternative Flows | If the user selects a route of a different category, the system pre-fetches compare data for the new category. Previously fetched categories remain cached. |
| Exceptions | If the backend is unreachable or the compare button is tapped before pre-fetch completes, the system fetches synchronously with a loading spinner. |
| Includes | NIL |
